...
Each wave uses its own branch, a pull request into `main`, CI, and a Vercel preview. Do not deploy production before Wave 08.
...
- Scope: local stack once Docker is running, generated `src/lib/supabase/database.types.ts`, auth redirect check, seed against a real user, advisors
- Acceptance: RLS still on, service role stays out of the client, migrations apply on a local `db reset` without `--linked`
- Tests: migration apply plus a sync round-trip with a local user
- PR gate: security-reviewer on the diff
...
- Scope: production Supabase project, Vercel production env, real-device pass
