...
import { ArrowLeft, Dumbbell, Timer, Gauge, ChevronRight } from "lucide-react";
  ...
  return (
    <div className="page-stack">
      <header className="page-heading">
        <Link href={`/plan/week/${week.week_number}`} className="row small muted" style={{ width: "fit-content" }}><ArrowLeft size={16} /> Semaine {week.week_number}</Link>
        ...
        <div className="row wrap" style={{ marginTop: 4 }}>
          <span className="pill"><Dumbbell size={13} /> {allItems.length} items</span>
          <span className="pill"><Timer size={13} /> temps selon prescription</span>
          {allItems.some((item) => item.item_kind === "strength_test") ? <span className="pill pill-accent"><Gauge size={13} /> test de force</span> : null}
        </div>
      </header>
