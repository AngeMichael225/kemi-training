    ...
    <div className="page-stack">
      ...
      {highAnomalies.length ? (
        <div className="card card-pad row" style={{ alignItems: "flex-start" }}>
          <Icon name="exclamation" size={20} style={{ color: "var(--warning)", marginTop: 2 }} />
      ...
      <section className="stack">
        <div className="section-title"><h2 className="h2">Semaines disponibles</h2><span className="pill pill-accent"><Icon name="layers" size={13} /> {trainingSeed.program.weeks.length}</span></div>
        <div className="grid-auto">
          {trainingSeed.program.weeks.map((week) => (
            <Link href={`/plan/week/${week.week_number}`} key={week.id} className="card card-pad card-elevated stack" style={{ minHeight: 190 }}>
              <div className="row-between"><span className="eyebrow" style={{ fontSize: ".66rem" }}>Semaine {week.week_number}</span><Icon name="arrow-right" size={18} /></div>
