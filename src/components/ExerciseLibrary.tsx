        ...
        {filtered.map((item) => (
          <Link key={item.id} href={`/exercise/${item.slug}`} className="exercise-tile">
            <div className="exercise-thumb">
              {item.imageUrl ? <Image src={item.imageUrl} width={148} height={148} alt="" sizes="74px" style={{ width: "100%", height: "100%", objectFit: "cover" }} /> : <Icon name="dumbbell-fitness" size={24} className="muted" />}
            </div>
