    ...
    <div className="page-stack">
      ...
      {active ? (
        <Link href={`/session/${active.id}?workout=${active.workoutDayId}`} className="card card-pad card-elevated row-between" aria-label="Reprendre la séance active">
          ...
          <span className="icon-button" aria-hidden="true"><Icon name="play" size={19} /></span>
        </Link>
      ) : null}
      ...
      <section className="hero-card">
        ...
        <div className="hero-content">
          <div className="row-between" style={{ alignItems: "flex-start" }}>
            <span className="hero-badge"><Icon name="calendar" size={14} /> Semaine {week.week_number} / Jour {recommended.day_number}</span>
          </div>
          <div style={{ maxWidth: 520 }}>
            <div className="row wrap" style={{ marginBottom: 12 }}>
              <span className="pill pill-accent"><Icon name="dumbbell-fitness" size={13} /> {strengthCount} mouvements</span>
              {cardioCount ? <span className="pill"><Icon name="stopwatch" size={13} /> cardio inclus</span> : null}
            </div>
            ...
            <div className="row wrap">
              ...
              <Link href={`/workout/${recommended.id}`} className="button button-secondary">Voir le detail <Icon name="arrow-right" size={17} /></Link>
            </div>
          </div>
        </div>
      </section>
      ...
      <section className="stack">
        <div className="section-title"><h2 className="h2">Aperçu</h2><Link href={`/plan/week/${week.week_number}`} className="small muted row">Programme <Icon name="angle-small-right" size={16} /></Link></div>
        <div className="workout-list">
          {recommended.sections.slice(0, 3).map((section, index) => (
            <div className="workout-row" key={section.id}>
              ...
              <Icon name="angle-small-right" size={18} className="muted" />
            </div>
          ))}
        </div>
      </section>
