    ...
    <div className="stack">
      ...
      {hasWeight ? (
        <label className="label">Charge
          <div className="counter-control">
            <button type="button" className="icon-button" onClick={() => setWeight((value) => Math.max(0, (value ?? 0) - weightStep))} aria-label="Diminuer la charge"><Icon name="minus" size={18} /></button>
            <div className="counter-value">{weight === null ? "--" : `${weight} ${preferredUnit}`}</div>
            <button type="button" className="icon-button" onClick={() => setWeight((value) => (value ?? 0) + weightStep)} aria-label="Augmenter la charge"><Icon name="plus" size={18} /></button>
          </div>
        </label>
      ) : null}
      ...
      {hasReps ? (
        <label className="label">Répétitions
          <div className="counter-control">
            <button type="button" className="icon-button" onClick={() => setReps((value) => Math.max(0, (value ?? 0) - 1))} aria-label="Diminuer les répétitions"><Icon name="minus" size={18} /></button>
            <div className="counter-value">{reps ?? "--"} reps</div>
            <button type="button" className="icon-button" onClick={() => setReps((value) => (value ?? 0) + 1)} aria-label="Augmenter les répétitions"><Icon name="plus" size={18} /></button>
          </div>
        </label>
      ) : null}
      ...
      <button type="button" className="button button-primary" onClick={() => onComplete({ reps, weight, unit: hasWeight ? preferredUnit : null, durationSec: simpleTimed ? item.prescribed_duration_sec : null })}>
        <Icon name="check" size={19} /> Serie terminée
      </button>
    </div>
  );
}
